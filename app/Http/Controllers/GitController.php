...
class GitController extends Controller
{
    ...
    // 执行 git pull 和（根据参数）composer install 的方法
    public function updateProject(Request $request)
    {
        try {
            ...
            // 根据请求参数判断是否执行 composer install
            if ($request->input('run_composer', false)) {
                $composerProcess = new Process(['sudo', 'composer', 'install', '--no-ansi']);
                ...
                $composerOutput = $composerProcess->getOutput();
            } else {
